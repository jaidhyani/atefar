from atefar.sig_types import SigData
...
method_metric_results_json = SigData(
...
task_candidates_json = SigData(
    "task_candidates_json",
    f"""
    Given the core ideas and experimental methods from the paper, enumerate a list of potential tasks that 
    could be used to evaluate an engineer or AI agent's ability to implement these methods. Each task should
    ultimately take the form of an instruction to the engineer, a python function to be implemented, and a 
    scoring function to evaluate the correctness of the implementation.
    We will not be building the tasks themselves here, but rather considering possible tasks that could be 
    built from the paper's content and evaluating the feasibility of those tasks. 

    For each task, describe:
     - How the task could be objectively evaluated and assigned a score (e.g. in code without human judgment)
     - What part of the paper the task corresponds to
     - What inputs and outputs the task would require. These may be any type of data, model, or other asset.
     - What skills and knowledge the task would test
     - What assets should be given to the engineer to complete the task
     - The minimum hardware requirements for the task
     - The evaluation criteria for the task
     - A description of what baseline implementation, if any, should be provided to the engineer
     - A short summary of the instructions that will be given to the engineer to complete the task
     - The expected time to complete the task in hours
     - Difficulty of the task on a scale of 1-5 (1 being easy, 5 being hard)
     - Feasibility of implementing an objective scoring function for the task on a scale of 1-5 (1 being easy, 5 being hard)
     - Degree to which the task measures actual research ability on a scale of 1-5 (1 being low, 5 being high)

     Response should be a JSON list of objects, where each object represents a task candidate. For example:
    {{
        {{
...
task_prerequisites_json = SigData(
    "task_prerequisites_json",
    f"""
    For each task candidate, list the prerequisites that an engineer should have in order to complete the task.
    ...
    Response should be a JSON object where keys are task names and values are lists of prerequisites. For example:
    {{
...
task_eval_baseline_implementation_json = SigData(
    ...
    f"""
    For each task candidate, write the baseline implementation that will be provided to the engineer. 
...
task_eval_metric_scoring_json = SigData(
    ...
    f"""
    For each task, write a python function that will be used to score the metric results of the implementation. 
    ...
    Response should be a JSON object where keys are task names and values are the metric scoring functions as strings. For example:
...
task_eval_combined_scoring_json = SigData(
    "task_eval_combined_scoring_json",
    f"""
    For each task, write a python function that will be used to score the overall performance of the implementation.
